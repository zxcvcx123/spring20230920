...
-- chais 상품의 공급자명, 전화번호
SELECT p.SupplierID, ProductName, SupplierName, Phone
FROM products p
         JOIN suppliers s
              ON p.SupplierID = s.SupplierID
WHERE ProductName = 'chais';

-- 1996년 7월 4일 에 주문한 고객명
SELECT *
FROM customers;
SELECT *
FROM orders;
...
-- 1996년 7월 4일 에 주문을 담당한 직원명
SELECT *
FROM employees;
SELECT *
FROM orders;

SELECT e.EmployeeID, e.LastName, e.FirstName, o.OrderDate
FROM employees e
         JOIN orders o
...
SELECT c.CustomerName, CONCAT(e.LastName, ' ', e.FirstName) EmployeeName, o.OrderDate
FROM customers c
    JOIN orders o
        ON c.CustomerID = o.CustomerID
    JOIN employees e
